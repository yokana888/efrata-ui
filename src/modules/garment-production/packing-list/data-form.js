...
export class DataForm {
  ...
  get removeItems() {
    return (event) => {
      ...
      this.data.grossWeight = this.data.items.reduce(
        (acc, cur) => (acc += cur.avG_GW),
        0
      );
      this.data.nettWeight = this.data.items.reduce(
        (acc, cur) => (acc += cur.avG_NW),
        0
      );
      this.error = null;
    ...
    if (this.data.measurements) {
      for (var m of this.data.measurements) {
        if (m.length && m.width && m.height && m.cartonsQuantity) {
          total +=
            (m.length * m.width * m.height * m.cartonsQuantity) / 1000000;
        }
      }
    }
  ...
  updateMeasurements() {
    ...
    for (const measurementCarton of measurementCartons) {
      let measurement = measurements.find(
        (m) =>
          ...
          m.index == measurementCarton.index
      );
      if (measurement) {
    ...
    for (const mt of measurements) {
      let measurement = (this.data.measurementsTemp || []).find(
        (m) =>
          m.length == mt.length &&
          m.width == mt.width &&
          m.height == mt.height &&
