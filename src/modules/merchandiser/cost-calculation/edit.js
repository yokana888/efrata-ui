...
export class Edit {
  ...
  async activate(params) {
    ...
    if (this.data) {
      ...
      const prMasterIds = this.data.CostCalculationGarment_Materials.filter(
        (m, i) =>
          m.PRMasterId > 0 &&
          this.data.CostCalculationGarment_Materials.findIndex(
            (d) => d.PRMasterId === m.PRMasterId
          ) === i
      ).map((m) => `Id==${m.PRMasterId}`);

      const prMasterItemIds = this.data.CostCalculationGarment_Materials.filter(
        (item, index) =>
          item.PRMasterItemId > 0 &&
          this.data.CostCalculationGarment_Materials.findIndex(
            (d) => d.PRMasterItemId === item.PRMasterItemId
          ) === index
      ).map((item) => item.PRMasterItemId);

      //Find If data From PR
      if (prMasterIds.length > 0) {
        ...
        this.data.CostCalculationGarment_Materials.forEach((material) => {
          if (material.PRMasterItemId > 0) {
            const quantity = prMasters.find(
              (pr) => pr.PRMasterItemId === material.PRMasterItemId
            ).Quantity;
            const budgetQuantities = ccMaterials
              .filter((m) => m.PRMasterItemId === material.PRMasterItemId)
              .reduce((acc, cur) => acc + cur.BudgetQuantity, 0);
            material.AvailableQuantity = quantity - budgetQuantities;
          }
        });
      }

      //Find If data From DO Items
      const doItemsIds = this.data.CostCalculationGarment_Materials.filter(
        (m, i) =>
          m.DOItemsId > 0 &&
          this.data.CostCalculationGarment_Materials.findIndex(
            (d) => d.DOItemsId === m.DOItemsId
          ) === i
      ).map((m) => `Id==${m.DOItemsId}`);

      if (doItemsIds.length > 0) {
        let doItemsFilter = {};
        doItemsFilter[`(${doItemsIds.join("||")})`] = true;

        const prMasterResult = await this.prService.searchDOItems({
          size: 10000,
          filter: JSON.stringify(doItemsFilter),
        });
        let prMasters = [];
        for (const d of prMasterResult.data) {
          prMasters.push({
        ...
        this.data.CostCalculationGarment_Materials.forEach((material) => {
          if (material.PRMasterItemId > 0) {
            const DOItems = prMasters.find(
              (pr) => pr.DOItemsId === material.DOItemsId
            );
            material.AvailableQuantity = DOItems.RemainingQuantity;
          }
        });
      }
    }
