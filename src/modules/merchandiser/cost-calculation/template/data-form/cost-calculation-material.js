...
export class CostCalculationMaterial {
  ...
  activate(context) {
    ...
    this.data.showDialog =
      this.data.showDialog === undefined
    ...
    if (this.data.Category) {
      ...
      this.categoryIsExist =
        this.data.Category.name.toUpperCase() == "FABRIC" ? true : false;
    ...
    if (this.data.Id || this.data.isCopy) {
      if (
        ...
        this.data.Category.name.toUpperCase() !== "FABRIC"
      ) {
  ...
  async selectedCategoryChanged(newVal, oldVal) {
    ...
    if (newVal) {
      ...
      if (this.data.Category.name.toUpperCase() === "FABRIC") {
        ...
        this.dialog
          .prompt(
            "Apakah fabric ini menggunakan harga CMT?",
            "Detail Fabric Material"
          )
          .then((response) => {
            if (response == "ok") {
      ...
      if (this.selectedCategory && this.selectedCategory.name) {
        if (this.selectedComposition && this.selectedComposition.Composition) {
          if (
            ...
            this.selectedConstruction.Const &&
            this.selectedConstruction.Const.length > 0
          ) {
            if (
              this.selectedYarn &&
              this.selectedYarn.Yarn &&
              this.selectedYarn.Yarn.length > 0
            ) {
              filter = JSON.stringify({
                name: this.selectedCategory.name,
                Composition: this.selectedComposition.Composition,
                const: this.selectedConstruction.Const,
                yarn: this.selectedYarn.Yarn,
              });
            } else {
              filter = JSON.stringify({
                name: this.selectedCategory.name,
  ...
  get garmentProductLoader() {
    return (keyword) => {
      ...
      return this.service.getGarmentProducts(keyword, filter).then((result) => {
        return result;
      });
    };
  }
        ...
        if (this.selectedComposition && this.selectedComposition.Composition) {
          if (
            ...
            if (
              this.selectedYarn &&
              this.selectedYarn.Yarn &&
              this.selectedYarn.properties.Yarn > 0
            ) {
              filter = JSON.stringify({
                name: this.selectedCategory.name,
                Composition: this.selectedComposition.Composition,
  ...
  get total() {
    let total =
      this.data.Quantity && this.data.Conversion && parseFloat(this.data.Price)
        ? (parseFloat(this.data.Price) / this.data.Conversion) *
          this.data.Quantity
        : 0;
  ...
  get totalShippingFee() {
    let totalShippingFee = numeral(
      (this.data.ShippingFeePortion / 100) * this.data.TotalTemp
    ).format();
  ...
  @computedFrom(
    ...
    "data.Conversion",
    "data.QuantityOrder",
    "data.FabricAllowance",
    "data.AccessoriesAllowance"
  )
  get budgetQuantity() {
    let allowance = 0;
    if (this.data.Category) {
    ...
    let budgetQuantity =
      this.data.Quantity && this.data.Conversion
        ? (this.data.Quantity * this.data.QuantityOrder) /
            this.data.Conversion +
          (allowance * this.data.Quantity * this.data.QuantityOrder) /
            this.data.Conversion
        : 0;
    ...
    this.dialog
      ...
      .then((response) => {
        if (!response.wasCancelled) {
          ...
          this.serviceCore
            .getCategoryId(this.data.Category.Id)
            .then((category) => {
  ...
  clickDOItem() {
    ...
    this.dialog
      .show(DOItemDialog, {
        DOItemsIds: this.context.context.items || 0,
      })
      .then((response) => {
        if (!response.wasCancelled) {
          this.error = {};
          ...
          this.data.IsPRMaster = true;
          // this.data.PRMasterId = result.PRMasterId;
          this.data.DOItemsId = result.DOItemsId;
          this.data.PRMasterItemId = result.PRMasterItemId;
          this.data.POMaster = result.POMaster;
